...
<div class="container">
        <form action="{{route('editinfo')}}" method="POST">
            @csrf
                <input type="hidden" name="id" value="{{$customer_info}}">
                <p><b>Communication Date:</b></p>
                <input type="date pickers" name="communication_date" class="form-control" value="{{$customer_info->communication_date}}" required>
